...
export class CharacterPanel {
    constructor(sidecarManager, triggerAnalysis, triggerArchitect) {
        ...
        this.currentSelection = 'main'; // 'main' or NPC name
    ...
    updateNPCDropdown(sidecar, mainCharName) {
        ...
        if (this.npcList.length > 0) {
            optionsHTML += '<optgroup label="NPCs">';
        ...
        if (currentValue !== 'main' && !this.npcList.includes(currentValue)) {
    ...
    /**
     * Render content based on current selection (main character or NPC)
     */
    renderCurrentSelection() {
        if (!this.currentSidecar) {
            this.showEmpty();
            return;
        }
