...
export class CharacterPanel {
    ...
    renderStatus(status) {
        ...
        // Handle conditions that might be array, string, or other types
        let conditionsHtml = '';
        if (status.conditions) {
            let conditionsText;
            if (Array.isArray(status.conditions) && status.conditions.length > 0) {
                conditionsText = status.conditions.join(', ');
            } else if (typeof status.conditions === 'string' && status.conditions) {
                conditionsText = status.conditions;
            } else if (typeof status.conditions === 'object') {
                conditionsText = JSON.stringify(status.conditions);
            }
            if (conditionsText) {
                conditionsHtml = `
                    <div class="sidecar-data-row">
                        <span class="sidecar-data-label">Conditions</span>
                        <span class="sidecar-data-value">${conditionsText}</span>
                    </div>
                `;
            }
        }
