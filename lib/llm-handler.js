...
If nothing significant changed, return empty operations array.`;
     ...
     * @param {Object} npcRegistry - The NPC registry data
     * @returns {string} Formatted text describing known NPCs
     */
    formatNPCRegistryForPrompt(npcRegistry) {
        if (!npcRegistry || !npcRegistry.npcs || Object.keys(npcRegistry.npcs).length === 0) {
        ...
        for (const [npcId, npc] of Object.entries(npcRegistry.npcs)) {
        ...
        // Scene context
        if (npcRegistry.sceneContext?.activeNPCs?.length > 0) {
            lines.push(`\n[CURRENT SCENE NPCs]: ${npcRegistry.sceneContext.activeNPCs.join(', ')}`);
        }
