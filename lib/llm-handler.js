...
Your goal is to analyze the latest story developments and output MINIMAL JSON operations to update the character's state.

IMPORTANT: Keep updates minimal. Only update what actually changed.
- ONE operation per variable that changed
- NO duplicate operations for the same path
- AVOID redundant updates (don't update what's already tracked)
- MAX 1-3 operations per character/NPC unless major events occurred
...
- add: { "op": "add", "path": "trackers.inventory.bag", "value": "New Item" }
  Use for adding ONE item to arrays (call once per unique item)
...
- append: { "op": "append", "path": "timeline", "value": { "turn": 5, "event": "..." } }
  Use for always adding to arrays (timeline events - ONE per significant event)
...
ONLY include NPCs who had meaningful interactions or changes in this segment.
...
For MINOR NPCs, provide MAX 1-2 updates:
- data.notes - Brief description (only if newly encountered or description changed)
- data.sentiment - Their attitude toward the main character (only if changed)

For MAJOR NPCs, provide ONLY changed fields (typically 1-3 updates):
- trackers.status.mood - Emotional state (only if mood changed in scene)
- trackers.personality.traits - ONE trait per add operation (only newly revealed traits)
- trackers.personality.goals - ONE goal per add (only newly revealed goals)
- trackers.knowledge - ONE fact per add (only newly learned info)
- narrativeRole.archetype - Character archetype (only if newly determined)
- timeline - ONE event summary (only major events involving this NPC)

Rules:
1. MINIMAL UPDATES: Only output operations for things that actually changed
2. NO DUPLICATES: Never add the same value to the same path twice
3. ONE PER PATH: Use ONE operation per path (combine info if needed)
4. BE CONCISE: Short values - "angry" not "feeling angry and upset"
5. SKIP UNCHANGED: If nothing changed, output empty arrays
6. NEW NPCs: Default to "minor" unless clearly plot-significant
7. QUALITY OVER QUANTITY: Better to track 1 important change than 5 trivial ones

Output Format (JSON only, no markdown):
{
  "operations": [
    { "op": "set", "path": "trackers.status.mood", "value": "tired" }
  ],
  "npcClassifications": [
    {
      "name": "Bob",
      "currentClassification": "new",
      "recommendedClassification": "minor",
      "reasoning": "Brief one-time encounter",
      "sceneRelevant": true,
      "sentiment": "friendly",
      "updates": [
        { "op": "set", "path": "data.notes", "value": "Tavern keeper who served drinks" }
      ]
    }
  ],
  "sceneNPCs": ["Bob"],
...
Example with NO changes (most common):
{
  "operations": [],
  "npcClassifications": [],
  "sceneNPCs": [],
...
If nothing significant changed, return empty operations array.`;
     ...
     * Also deduplicates updates within each NPC
     */
    normalizeNPCClassifications(npcData) {
        ...
        return npcData.map(npc => {
            ...
            // Also deduplicate updates from LLM
            const updates = this.deduplicateNPCUpdates(npc.updates || []);
    ...
    /**
     * Deduplicate NPC update operations
     * Removes duplicate add/append operations with same path and value
     */
    deduplicateNPCUpdates(updates) {
        if (!Array.isArray(updates)) {
            return [];
        }
        
        const seen = new Map(); // "path:op" -> Set of JSON-stringified values
        const result = [];
        
        for (const update of updates) {
            if (!update.path) continue;
            
            // For add/append operations, deduplicate by path + value
            if (update.op === 'add' || update.op === 'append') {
                const key = `${update.path}:${update.op}`;
                const valueKey = typeof update.value === 'object'
                ...
                if (!seen.get(key).has(valueKey)) {
                    seen.get(key).add(valueKey);
                    result.push(update);
                }
            } else {
                // For set/remove/increment, keep last value for same path
                const existingIndex = result.findIndex(u => u.path === update.path && u.op === update.op);
                if (existingIndex >= 0) {
                    result[existingIndex] = update; // Replace with latest
                } else {
                    result.push(update);
                }
            }
