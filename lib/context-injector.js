...
export class ContextInjector {
    ...
    /**
     * Inject sidecar context using SillyTavern's extension prompt system
     * This properly injects context without modifying the actual chat array
     * @param {Array} chat - Chat message array (used for reference, not modified)
    ...
    async inject(chat, characterId, position = 'system', npcOptions = {}) {
        ...
        // Use SillyTavern's extension prompt system instead of modifying chat array
        const context = SillyTavern.getContext();
        
        // Map our position to SillyTavern's extension_prompt_types
        // SillyTavern positions: 0=AFTER_SCENARIO, 1=IN_CHAT, 2=BEFORE_PROMPT, etc.
        let stPosition = 1; // Default: IN_CHAT
        let depth = 4;      // How many messages from the end to inject
        
        switch (position) {
            case 'system':
                stPosition = 0; // AFTER_SCENARIO - near system prompt
                depth = 0;
                break;
            case 'author_note':
                stPosition = 1; // IN_CHAT - like author's note
                depth = 4;      // 4 messages from end (typical author's note depth)
                break;
            case 'character_note':
                stPosition = 0; // AFTER_SCENARIO - near character info
                depth = 0;
                break;
            default:
                stPosition = 1;
        ...
        // Use setExtensionPrompt if available (SillyTavern API)
        if (typeof context.setExtensionPrompt === 'function') {
            context.setExtensionPrompt(
                'sidecar-lore',           // Extension identifier
                fullContext,              // The content to inject
                stPosition,               // Position in prompt
                depth,                    // Depth (messages from end)
                false,                    // Not scannable by AI
                0                         // Role: 0=system
            );
            console.log(`[ContextInjector] Injected via setExtensionPrompt at position ${position}`);
        } else {
            // Fallback: Log what would be injected (don't modify chat array)
            console.log(`[ContextInjector] Extension prompt API not available. Context prepared but not injected.`);
            console.log(`[ContextInjector] Content length: ${fullContext.length} chars`);
        }

        const npcCount = this.npcRegistry ? Object.keys(this.npcRegistry.getRegistryData()?.npcs || {}).length : 0;
        console.log(`[ContextInjector] Context prepared (NPCs: ${npcCount})`);
    }

    /**
     * Clear the sidecar extension prompt (call when chat changes or extension disabled)
     */
    clearInjection() {
        try {
            const context = SillyTavern.getContext();
            if (typeof context.setExtensionPrompt === 'function') {
                context.setExtensionPrompt('sidecar-lore', '', 0, 0, false, 0);
                console.log('[ContextInjector] Cleared extension prompt');
            }
        } catch (error) {
            console.warn('[ContextInjector] Could not clear extension prompt:', error);
        }
    }
