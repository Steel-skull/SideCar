...
export class ContextInjector {
    ...
    formatSidecarContext(sidecar, options = {}) {
        ...
        // Active quests (brief) - ensure active is an array before using map
        const activePlots = Array.isArray(sidecar.plotThreads?.active) ? sidecar.plotThreads.active : [];
        if (activePlots.length > 0) {
            const quests = activePlots.map(q => typeof q === 'object' ? q.name : q).filter(n => n);
            if (quests.length > 0) {
    ...
    /**
     * Inject sidecar context using SillyTavern's /inject slash command
     * This uses the script_injects system which properly handles context injection
     * @param {Array} chat - Chat message array (not modified)
    ...
    async inject(chat, characterId, position = 'system', npcOptions = {}) {
        ...
        if (!fullContext) {
            console.log('[ContextInjector] No context to inject');
            return; // Nothing to inject
        }

        // Calculate depth based on position
        let depth = 4; // Default: 4 messages from end (like author's note)
        let role = 'system';
        
        switch (position) {
            case 'system':
                depth = 0; // At the very end (most recent context)
                role = 'system';
                break;
            case 'author_note':
                depth = 4; // 4 messages from end (typical author's note depth)
                role = 'system';
                break;
            case 'character_note':
                depth = 0;
        ...
        try {
            ...
            // Use the /inject slash command via executeSlashCommandsWithOptions
            // This is how other extensions like GuidedGenerations inject content
            // The injection is ephemeral (not saved with chat) and will be included in the prompt
            const injectCommand = `/inject id=sidecar-lore position=chat ephemeral=true scan=false depth=${depth} role=${role} ${fullContext}`;
            ...
            console.log(`[ContextInjector] Injected via /inject command (depth=${depth}, role=${role}, NPCs: ${npcCount})`);
        } catch (error) {
            console.error('[ContextInjector] Failed to inject via slash command:', error);
            
            // Log what we tried to inject for debugging
            console.log('[ContextInjector] Attempted to inject:', fullContext.substring(0, 200) + '...');
        }
    }

    /**
     * Clear any previously injected sidecar context
     * Uses /flushinject to remove the ephemeral injection
     */
    async clearInjection() {
        try {
            const context = SillyTavern.getContext();
            
            // Use /flushinject to remove the sidecar-lore injection
            await context.executeSlashCommandsWithOptions('/flushinject sidecar-lore', {
                displayCommand: false,
                showOutput: false
            });
            
            console.log('[ContextInjector] Cleared sidecar-lore injection');
        } catch (error) {
            // Don't log error if it's just that there was nothing to flush
            if (!error.message?.includes('not found')) {
                console.error('[ContextInjector] Failed to clear injection:', error);
            }
        }
    }
