...
export class ContextInjector {
    ...
    /**
     * Inject sidecar context into the chat array for prompt generation
     * This is called by the prompt interceptor and modifies the chat array directly
     * @param {Array} chat - Chat message array (will be modified)
    ...
    async inject(chat, characterId, position = 'system', npcOptions = {}) {
        ...
        switch (position) {
            case 'system':
                depth = chat.length; // Insert near the beginning
                break;
            case 'author_note':
                depth = 4; // 4 messages from end (typical author's note depth)
                break;
            case 'character_note':
                depth = chat.length - 1; // Near the beginning but after first message
                break;
            default:
                depth = 4;
        }

        // Create the sidecar message object
        // This is injected into the chat array and will be included in the prompt
        const sidecarMessage = {
            ...
            send_date: Date.now(),
            extra: {
                type: 'sidecar_injection',
                isSmallSys: true // Mark as small system message
            }
        };

        // Calculate insertion position
        // Depth is how many messages from the end to insert
        const insertPosition = Math.max(0, chat.length - depth);
        
        // Insert the sidecar message into the chat array
        chat.splice(insertPosition, 0, sidecarMessage);
        
        const npcCount = this.npcRegistry ? Object.keys(this.npcRegistry.getRegistryData()?.npcs || {}).length : 0;
        console.log(`[ContextInjector] Injected at position ${insertPosition} (depth=${depth}, NPCs: ${npcCount})`);
    }

    /**
     * Clear any sidecar injections from the chat array
     * Call this when chat changes or extension is disabled
     * @param {Array} chat - Chat message array to clean
     */
    clearInjection(chat) {
        ...
        // Remove any sidecar injection messages from the chat
        for (let i = chat.length - 1; i >= 0; i--) {
            const msg = chat[i];
            if (msg.extra && msg.extra.type === 'sidecar_injection') {
                chat.splice(i, 1);
            }
        }
        console.log('[ContextInjector] Cleared sidecar injections from chat');
    }
