 ...
 * Also deduplicates operations targeting the same array path with same value
 *
...
function remapOperationsForClassification(operations, classification) {
    ...
    const remapped = operations.map(op => {
        const remappedOp = { ...op };
    ...
    // Deduplicate operations that add the same value to the same array path
    return deduplicateArrayOperations(remapped);
}

/**
 * Deduplicate operations targeting array paths with the same value
 * Keeps first occurrence and removes duplicates
 *
 * @param {Object[]} operations - Array of delta operations
 * @returns {Object[]} Deduplicated operations
 */
function deduplicateArrayOperations(operations) {
    const seen = new Map(); // path -> Set of JSON-stringified values
    const result = [];
    
    for (const op of operations) {
        // Only deduplicate 'add' and 'append' operations on array paths
        if (op.op === 'add' || op.op === 'append') {
            const valueKey = typeof op.value === 'object'
                ? JSON.stringify(op.value)
                : String(op.value);
            
            const key = `${op.path}:${op.op}`;
            ...
            if (!seenValues.has(valueKey)) {
                ...
                console.log(`[NPCClassifier] Deduplicating: ${op.op} to ${op.path}: ${valueKey}`);
            }
        } else {
            // Non-array operations pass through
            result.push(op);
        }
    }
